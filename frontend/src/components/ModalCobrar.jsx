...
function ModalCobrar({ deuda, onCerrar, onCobrado }) {
  ...
  const saldoAFavor   = montoFinal > montoSugerido ? montoFinal - montoSugerido : 0
  ...
  async function cobrar() {
    ...
    try {
      await api.post(`/deudas/${deuda.deuda_id}/pagar`, {
        monto:       montoFinal,
        metodo_pago: metodo,
        con_recargo: recargo > 0,  // ← indica si el excedente es recargo por mora
      })
      onCobrado()
      onCerrar()
    } catch(e) {
    ...
    <div onClick={onCerrar}
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1.25rem" }}>
          <div>
            <p style={{ fontSize:"15px", fontWeight:500, color: TEMA.textoPrimario, margin:0 }}>Registrar pago</p>
        ...
        <div style={{ background:"#1a1a1a", border:`0.5px solid ${TEMA.borde}`, borderRadius:"6px", padding:"10px 12px", marginBottom:"12px" }}>
          ...
          {tipoRecargo !== "ninguno" && (
            <input value={valorRecargo}
              onChange={e => { setValorRecargo(e.target.value.replace(/\D/g, "")); setMontoPagado("") }}
              placeholder={tipoRecargo === "monto" ? "Ej: 500" : "Ej: 10"} inputMode="numeric"
        ...
        <div style={{ marginBottom:"12px" }}>
          ...
          <input value={montoPagado}
            onChange={e => setMontoPagado(e.target.value.replace(/\D/g, ""))}
            placeholder={`$${montoSugerido.toLocaleString("es-AR")} (sugerido)`} inputMode="numeric"
        ...
        {montoFinal > 0 && (
          <div style={{ marginBottom:"12px", padding:"10px 12px", borderRadius:"6px",
            ...
            {tipoPago === "excedente" && recargo > 0 && (
              <>
                <p style={{ fontSize:"12px", color: TEMA.primarioHover, margin:"0 0 2px", fontWeight:500 }}>Pago con recargo</p>
                <p style={{ fontSize:"11px", color: TEMA.textoTerciario, margin:0 }}>
                  Recargo: <span style={{ color: TEMA.primarioHover }}>{formatPeso(recargo)}</span>
                  {saldoAFavor > 0 && <> · Saldo a favor: <span style={{ color:"#66aaff" }}>{formatPeso(saldoAFavor)}</span></>}
                </p>
              </>
            )}
            {tipoPago === "excedente" && recargo === 0 && (
        ...
        {error && <p style={{ fontSize:"12px", color: TEMA.primarioHover, marginTop:"8px" }}>{error}</p>}
        ...
        <div style={{ display:"flex", gap:"8px", marginTop:"1.25rem" }}>
          ...
          <button onClick={cobrar} disabled={cargando || montoFinal <= 0}
